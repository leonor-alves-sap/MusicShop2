...
class RentalRepository {
    ...
    public async getAllRentals(): Promise<Rental[]> {
        ...
        return result.rows.map((rentalData) => new Rental(
  ...
  public async getRentalsByClient(client_id: string): Promise<Rental[]> {
    ...
    try {
      const result = await client.query('SELECT * FROM rentals WHERE client_id=$1', [client_id]);
      return result.rows.map((rentalData) => new Rental(
        rentalData.client_id,
        rentalData.vinyl_id,
        rentalData.rental_date,
  ...
  public async updateRentalByClient(email: string, rental: Rental): Promise<Rental | null> {
