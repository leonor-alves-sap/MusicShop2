...
class ClientRepository {
  ...
  public async createClient(client: Client): Promise<void> {
    ...
    try {
      await dbClient.query(
        'INSERT INTO clients (name, email, age, gender, balance) VALUES ($1, $2, $3, $4, $5)',
        [name, email, age, gender, balance]
      );
    } catch (error) {
  ...
  public async getAllClients(): Promise<Client[]> {
    ...
    try {
      ...
      return result.rows.map((clientData) => new Client(
        clientData.name,
        clientData.email,
        clientData.age,
        clientData.gender,
  ...
  public async getClientByEmail(email: string): Promise<Client | null> {
    ...
    try {
      const result = await dbClient.query('SELECT * FROM clients WHERE email = $1', [email]);
  ...
  public async updateClientByEmail(email: string, updatedClient: Client): Promise<Client> {
    const dbClient = await pool.connect();
  ...
  public async updateClientBalanceByEmail(email: string, balance: number): Promise<void> {
    const dbClient = await pool.connect();

    try {
      await dbClient.query(
        'UPDATE clients SET balance = $1 WHERE email = $2',
        [balance, email]
      );
    } catch (error) {
