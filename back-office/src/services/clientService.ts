...
class ClientService {
    ...
    public async updateClientBalance(email: string, newBalance: number): Promise<Client | null> {

        const clientPromise: Promise<Client | null> = clientRepository.getClientByEmail(email);
    ...
    public async insertClient(client: Client): Promise<Client | null> {
        ...
        const clientPromise: Promise<Client | null> = clientRepository.getClientByEmail(client.getEmail());
        const existingClient = await clientPromise;
